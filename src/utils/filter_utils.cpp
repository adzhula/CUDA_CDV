...
namespace cuda_filter
{
    ...
    cv::Mat FilterUtils::createFilterKernel(FilterType type, int kernelSize, float intensity)
    {
        ...
        case FilterType::HDR_TONEMAPPING:
            ...
            if (kernelSize >= 3)
            {
                float centerValue = 1.0f + 2.0f * intensity;
                float edgeValue = -0.25f * intensity;
                float cornerValue = -0.125f * intensity;

                kernel.at<float>(1, 1) = centerValue;

                kernel.at<float>(0, 1) = edgeValue;
                kernel.at<float>(1, 0) = edgeValue;
                kernel.at<float>(1, 2) = edgeValue;
                kernel.at<float>(2, 1) = edgeValue;

                kernel.at<float>(0, 0) = cornerValue;
                kernel.at<float>(0, 2) = cornerValue;
                kernel.at<float>(2, 0) = cornerValue;
                kernel.at<float>(2, 2) = cornerValue;
